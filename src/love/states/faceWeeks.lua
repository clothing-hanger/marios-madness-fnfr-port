...
return {
	enter = function(self)
		...
		sounds = {
			...
			powWarn = {
				warn = love.audio.newSource("sounds/warn.ogg", "static")
			},
		...
		PDstatic = love.filesystem.load("sprites/powerdown/static.lua")()

		PDstatic.sizeX, PDstatic.sizeY = 1, 1

		PDstatic:setAnimSpeed(6)

		PDstatic.x, PDstatic.y = 640, 360

		lava.y = 700

		love.graphics.setDefaultFilter("nearest")
		pow = love.filesystem.load("sprites/powerdown/pow.lua")()
		pow.sizeX, pow.sizeY = 10, 10
		love.graphics.setDefaultFilter("linear")

		pow:animate("idle", false)
		lava:animate("anim", true)
		PDstatic:animate("anim", true)

		
		PDstatic:setAnimSpeed(15)
	...
	update = function(self, dt)
		...
		if input:pressed("shoulderButtonDodge") then boyfriend:animate("dodge", false) end
		...
		love.graphics.push()
			...
			if not settings.suddenDeath then
				if settings.downscroll then
					...
					elseif weekNumber == "powerdown" then
